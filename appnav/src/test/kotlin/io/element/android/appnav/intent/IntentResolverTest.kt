...
package io.prism.android.appnav.intent
...
import io.prism.android.features.login.test.FakeLoginIntentResolver
import io.prism.android.features.share.api.ShareIntentData
import io.prism.android.features.share.api.UriToShare
import io.prism.android.features.share.test.FakeShareIntentHandler
...
import io.prism.android.libraries.oidc.test.FakeOidcIntentResolver
...
class IntentResolverTest {
    ...
    fun `test resolve oidc`() {
        ...
        val intent = Intent(RuntimeEnvironment.getApplication(), Activity::class.java).apply {
            action = Intent.ACTION_VIEW
            data = "io.prism.android:/?error=access_denied&state=IFF1UETGye2ZA8pO".toUri()
        }
        val result = sut.resolve(intent)
        assertThat(result).isEqualTo(
            ResolvedIntent.Oidc(
    ...
    fun `test resolve external permalink`() {
        ...
        val intent = Intent(RuntimeEnvironment.getApplication(), Activity::class.java).apply {
            action = Intent.ACTION_VIEW
            data = "https://prism.to/#/@alice:prism.org".toUri()
        }
        val result = sut.resolve(intent)
        assertThat(result).isEqualTo(
    ...
    fun `test resolve external permalink, FallbackLink should be ignored`() {
        val sut = createIntentResolver(
            permalinkParserResult = { PermalinkData.FallbackLink(Uri.parse("https://prism.org")) },
            loginIntentResolverResult = { null },
            oidcIntentResolverResult = { null },
        )
        val intent = Intent(RuntimeEnvironment.getApplication(), Activity::class.java).apply {
    ...
    fun `test resolve external permalink, invalid action`() {
        ...
        val intent = Intent(RuntimeEnvironment.getApplication(), Activity::class.java).apply {
            ...
            data = "https://prism.to/invalid".toUri()
        }
        val result = sut.resolve(intent)
        assertThat(result).isNull()
    }
