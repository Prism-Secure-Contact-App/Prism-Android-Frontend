...
import io.prism.android.appnav.intent.ResolvedIntent
import io.prism.android.appnav.room.RoomFlowNode
import io.prism.android.appnav.room.RoomNavigationTarget
import io.prism.android.appnav.root.RootNavStateFlowFactory
import io.prism.android.appnav.root.RootPresenter
import io.prism.android.appnav.root.RootView
...
import io.prism.android.features.rageshake.api.bugreport.BugReportEntryPoint
...
import io.prism.android.libraries.architecture.appyx.rememberDelegateTransitionHandler
...
class RootFlowNode(
    ...
    override fun onBuilt() {
        ...
        appCoroutineScope.launch {
            prismSessionCache.restoreWithSavedState(buildContext.savedStateMap)
    ...
    override fun onSaveInstanceState(state: MutableSavedStateMap) {
        ...
        prismSessionCache.saveIntoSavedState(state)
                ...
                when (navState.loggedInState) {
                    is LoggedInState.LoggedIn -> {
                        if (navState.loggedInState.isTokenValid) {
                            ...
                            if (prismSessionCache.getOrNull(sessionId) != null) {
    ...
    private suspend fun restoreSessionIfNeeded(
        ...
        prismSessionCache.getOrRestore(sessionId).onSuccess {
        ...
        if (accountProviderAccessControl.isAllowedToConnectToAccountProvider(params.accountProvider.ensureProtocol())) {
            ...
            if (sessions.isNotEmpty()) {
                if (featureFlagService.isFeatureEnabled(FeatureFlags.MultiAccount)) {
                    val loginHintPRISMId = params.loginHint?.removePrefix("mxid:")
                    val existingAccount = sessions.find { it.userId == loginHintPRISMId }
