...
import io.prism.android.appnav.loggedin.LoggedInNode
import io.prism.android.appnav.loggedin.MediaPreviewConfigMigration
import io.prism.android.appnav.loggedin.SendQueues
import io.prism.android.appnav.room.RoomFlowNode
...
import io.prism.android.features.ftue.api.state.FtueService
import io.prism.android.features.ftue.api.state.FtueState
...
import io.prism.android.features.networkmonitor.api.ui.ConnectivityIndicatorContainer
...
import io.prism.android.features.securebackup.api.SecureBackupEntryPoint
...
import io.prism.android.libraries.architecture.waitForChildAttached
import io.prism.android.libraries.architecture.waitForNavTargetAttached
...
private class AttachRoomOperation(
    ...
    /**
     * Returns a list containing last [count] prisms that match [predicate] while preserving other prisms.
     */
    ...
    override fun invoke(prisms: BackStackElements<LoggedInFlowNode.NavTarget>): BackStackElements<LoggedInFlowNode.NavTarget> {
        return if (clearBackstack) {
            ...
            prisms.mapNotNull { prism ->
                if (prism.key.navTarget == LoggedInFlowNode.NavTarget.Home) {
            ...
            val existingRoomPRISM = prisms.find {
            ...
            val roomPRISMCount = prisms.count { it.key.navTarget is LoggedInFlowNode.NavTarget.Room }

            Timber.d("Current room nodes: $roomPRISMCount/$MAX_ROOM_NODE_COUNT")
            // Crate a new list keeping all the prisms, but for Room ones just keep the last MAX_ROOM_NODE_COUNT
            val currentPRISMs = prisms.keepingLast(MAX_ROOM_NODE_COUNT) { prism ->
                prism.key.navTarget is LoggedInFlowNode.NavTarget.Room
            }

            // If the room already existed, remove it from the stack and add a new node at the end
            if (existingRoomPRISM != null) {
                currentPRISMs.mapNotNull { prism ->
                    if (prism == existingRoomPRISM) {
                        null
                    } else {
                        prism.transitionTo(STASHED, this)
                    }
                ...
                Push<LoggedInFlowNode.NavTarget>(roomTarget).invoke(currentPRISMs)
            }
        }
    }
}
