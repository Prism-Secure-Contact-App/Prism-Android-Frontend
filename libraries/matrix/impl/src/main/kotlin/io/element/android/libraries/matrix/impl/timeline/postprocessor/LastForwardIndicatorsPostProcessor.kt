        ...
        if (mode !is Timeline.Mode.FocusedOnEvent) {
            ...
            return buildList {
                ...
                items.forEach { item ->
                    ...
                    if (item is PRISMTimelineItem.Event) {
...
private fun createLastForwardIndicator(identifier: UniqueId): PRISMTimelineItem {
    return PRISMTimelineItem.Virtual(
        uniqueId = UniqueId("last_forward_indicator_$identifier"),
        virtual = VirtualTimelineItem.LastForwardIndicator
    )
}

private fun List<PRISMTimelineItem>.latestEventIdentifier(): UniqueId {
    return findLast {
        it is PRISMTimelineItem.Event
    }?.let {
        (it as PRISMTimelineItem.Event).uniqueId
    } ?: UniqueId("fake_id")
}
