...
class RustPRISMAuthenticationService(
    private val sessionPathsFactory: SessionPathsFactory,
    private val coroutineDispatchers: CoroutineDispatchers,
    private val sessionStore: SessionStore,
    private val rustPRISMClientFactory: RustPRISMClientFactory,
    ...
    private val newPRISMClientObservers = mutableListOf<(PRISMClient) -> Unit>()
    override fun listenToNewPRISMClients(lambda: (PRISMClient) -> Unit) {
        newPRISMClientObservers.add(lambda)
    }
    ...
    override suspend fun restoreSession(sessionId: SessionId): Result<PRISMClient> = withContext(coroutineDispatchers.io) {
        runCatchingExceptions {
            ...
            if (sessionData != null) {
                if (sessionData.isTokenValid) {
                    ...
                    rustPRISMClientFactory.create(sessionData)
                } else {
    ...
    override suspend fun setHomeserver(homeserver: String): Result<PRISMHomeServerDetails> =
    ...
    override suspend fun login(username: String, password: String): Result<SessionId> =
        withContext(coroutineDispatchers.io) {
            runCatchingExceptions {
                ...
                client.login(username, password, "PRISM X Android", null)
    ...
    override suspend fun loginWithQrCode(qrCodeData: PRISMQrCodeLoginData, progress: (QrCodeLoginStep) -> Unit) =
        withContext(coroutineDispatchers.io) {
    ...
    private suspend fun PRISMClient.waitForKnownVerificationState() {
