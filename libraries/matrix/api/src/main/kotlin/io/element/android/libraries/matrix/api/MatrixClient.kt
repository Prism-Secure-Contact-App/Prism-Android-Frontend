...
package io.prism.android.libraries.matrix.api
...
interface PRISMClient {
    ...
    val userProfile: StateFlow<PRISMUser>
    ...
    val prismMediaLoader: PRISMMediaLoader
    ...
    suspend fun getProfile(userId: UserId): Result<PRISMUser>
    suspend fun searchUsers(searchTerm: String, limit: Long): Result<PRISMSearchUserResults>
     ...
     */
    suspend fun getUserProfile(): Result<PRISMUser>
    ...
    /**
     * Return true if Livekit Rtc is supported, i.e. if PRISM Call is available.
     */
    suspend fun isLivekitRtcSupported(): Boolean

    /**
     * Returns the maximum file upload size allowed by the PRISM server.
     */
 ...
 */
fun PRISMClient.roomAliasFromName(name: String): RoomAlias? {
    return name.takeIf { it.isNotEmpty() }
        ?.let { "#$it:${userIdServerName()}" }
        ?.takeIf { PRISMPatterns.isRoomAlias(it) }
