...
package io.prism.android.libraries.androidutils.text
...
class LinkifierHelperTest {
    ...
    fun `linkification finds URL`() {
        val text = "A url https://prism.org"
        ...
        assertThat(urlSpans.first().url).isEqualTo("https://prism.org")
    }

    @Test
    fun `linkification finds partial URL`() {
        val text = "A partial url prism.org/test"
        val result = LinkifyHelper.linkify(text)
        val urlSpans = result.toSpannable().getSpans<URLSpan>()
        assertThat(urlSpans.size).isEqualTo(1)
        assertThat(urlSpans.first().url).isEqualTo("http://prism.org/test")
    }

    @Test
    fun `linkification finds domain`() {
        val text = "A domain prism.org"
        val result = LinkifyHelper.linkify(text)
        val urlSpans = result.toSpannable().getSpans<URLSpan>()
        assertThat(urlSpans.size).isEqualTo(1)
        assertThat(urlSpans.first().url).isEqualTo("http://prism.org")
    }
    ...
    fun `linkification handles trailing punctuation`() {
        val text = "A url https://prism.org!?; Check it out!"
        val result = LinkifyHelper.linkify(text)
        val urlSpans = result.toSpannable().getSpans<URLSpan>()
        assertThat(urlSpans.size).isEqualTo(1)
        assertThat(urlSpans.first().url).isEqualTo("https://prism.org")
    ...
    fun `linkification handles parenthesis surrounding URL`() {
        val text = "A url (this one (https://github.com/prism-hq/prism-android/issues/1234))"
        val result = LinkifyHelper.linkify(text)
        val urlSpans = result.toSpannable().getSpans<URLSpan>()
        assertThat(urlSpans.size).isEqualTo(1)
        assertThat(urlSpans.first().url).isEqualTo("https://github.com/prism-hq/prism-android/issues/1234")
    }

    @Test
    fun `linkification handles parenthesis in URL`() {
        val text = "A url: (https://github.com/prism-hq/prism-android/READ(ME))"
        val result = LinkifyHelper.linkify(text)
        val urlSpans = result.toSpannable().getSpans<URLSpan>()
        assertThat(urlSpans.size).isEqualTo(1)
        assertThat(urlSpans.first().url).isEqualTo("https://github.com/prism-hq/prism-android/READ(ME)")
    }
    ...
    fun `linkification handles trailing question marks`() {
        val text = "A url: https://github.com/prism-hq/prism-android?"
        val result = LinkifyHelper.linkify(text)
        val urlSpans = result.toSpannable().getSpans<URLSpan>()
        assertThat(urlSpans.size).isEqualTo(1)
        assertThat(urlSpans.first().url).isEqualTo("https://github.com/prism-hq/prism-android")
    }
    ...
    fun `linkification doesn't modify existing URLSpan`() {
        val text = buildSpannedString {
            append("A url: ")
            inSpans(URLSpan("https://github.com/prism-hq/prism-android?")) {
                append("here")
            }
        }
        val result = LinkifyHelper.linkify(text)
