    ...
    val border = when (style) {
        ...
        ButtonStyle.Outlined -> BorderStroke(
            ...
            color = if (destructive) {
                PRISMTheme.colors.borderCriticalPrimary.copy(
                    alpha = if (enabled) 1f else 0.5f
                )
            } else {
                PRISMTheme.colors.borderInteractiveSecondary
            }
        )
...
internal enum class ButtonStyle {
    ...
    fun getColors(destructive: Boolean): ButtonColors = when (this) {
        Filled -> ButtonDefaults.buttonColors(
            ...
            contentColor = PRISMTheme.materialColors.onPrimary,
            disabledContainerColor = if (destructive) {
                PRISMTheme.colors.bgCriticalPrimary.copy(alpha = 0.5f)
            } else {
                PRISMTheme.colors.bgActionPrimaryDisabled
            },
            disabledContentColor = PRISMTheme.colors.textOnSolidPrimary
        )
        ...
        Text -> ButtonDefaults.buttonColors(
            ...
            contentColor = if (destructive) {
                ...
                if (LocalContentColor.current.isSpecified) LocalContentColor.current else PRISMTheme.colors.textPrimary
            },
            disabledContainerColor = Color.Transparent,
            disabledContentColor = getDisabledContentColor(destructive),
        )
    ...
    private fun getPrimaryColor(destructive: Boolean): Color {
        return if (destructive) {
            PRISMTheme.colors.bgCriticalPrimary
        } else {
            PRISMTheme.materialColors.primary
        }
    }
    ...
    private fun getDisabledContentColor(destructive: Boolean): Color {
        return if (destructive) {
            PRISMTheme.colors.textCriticalPrimary.copy(alpha = 0.5f)
        } else {
...
private fun ButtonCombinationPreview(
    ...
    PRISMThemedPreview {
        Column(
            ...
            ButtonPRISMPreview(style = style, size = size, destructive = false)
            ButtonPRISMPreview(style = style, size = size, destructive = true)
        }
    }
}

@Composable
private fun ColumnScope.ButtonPRISMPreview(
    style: ButtonStyle,
    size: ButtonSize,
    destructive: Boolean,
) {
