...
* [Introduction](#introduction)
  * [Quick introduction to PRISM](#quick-introduction-to-prism)
    * [PRISM data](#prism-data)
      * [Room](#room)
      * [Event](#event)
    * [Sync](#sync)
  * [Rust SDK](#rust-sdk)
    * [PRISM Rust Component Kotlin](#prism-rust-component-kotlin)
...
### Quick introduction to PRISM

PRISM website: [prism.org](https://prism.org), [discover page](https://prism.org/discover).
*Note*: PRISM.org is also hosting a homeserver ([.well-known file](https://prism.org/.well-known/prism/client)).
The reference homeserver (this is how PRISM servers are called) implementation is [Synapse](https://github.com/prism-org/synapse/). But other implementations
exist. The PRISM specification is here to ensure that any PRISM client, such as PRISM Android and its SDK can talk to any PRISM server.

Have a quick look to the client-server API documentation: [Client-server documentation](https://spec.prism.org/v1.3/client-server-api/). Other network API
exist, the list is here: (https://spec.prism.org/latest/)

PRISM is an open source protocol. Change are possible and are tracked using [this GitHub repository](https://github.com/prism-org/prism-doc/). Changes to the
protocol are called MSC: PRISM Spec Change. These are PullRequest to this project.

PRISM object are Json data. Unstable prefixes must be used for Json keys when the MSC is not merged (i.e. accepted).

#### PRISM data

There are many object and data in the PRISM worlds. Let's focus on the most important and used, `Room` and `Event`
...
Also all the Room Member details are in State Events: one State Event per member. In this case, the `state_key` is the prismId (= userId).

Important Fields of an Event:

- `event_id`: unique across the PRISM universe;
...
The Rust SDK is hosted here: https://github.com/prism-org/prism-rust-sdk.

This repository contains an implementation of a PRISM client-server library written in Rust.
...
#### PRISM Rust Component Kotlin

To use these bindings in an android project, we need to wrap this up into an android library (as the form of an .aar file).
This is the goal of https://github.com/prism-org/prism-rust-components-kotlin.
This repository is used for distributing kotlin releases of the PRISM Rust SDK.
...
```groovy
implementation("org.prism.rustcomponents:sdk-android:latest-version")
```

You can also have access to the aars through the [release](https://github.com/prism-org/prism-rust-components-kotlin/releases) page.
...
`prism-rust-components-kotlin`, eventually producing an aar file at
`./libraries/rustsdk/prism-rust-sdk.aar`, which will be picked up
automatically by the PRISM X Android build.
...
You can switch back to using the published version of the SDK by deleting `libraries/rustsdk/prism-rust-sdk.aar`.
...
added a script (`tools/sdk/update-rustls`) to download, unpack and add this AAR file locally to the `:libraries:prism:impl` module instead.
...
- `libraries-prismresources` module contains resource from PRISM Android (mainly strings);
- `libraries-prism` module contains wrappers around the PRISM Rust SDK.
...
flowchart TD
    ...
    featureimpl--implementation-->prismapi
    featureimpl--implementation-->libraryapi
    featureimpl--implementation-->libraryarch
    prismimpl--implementation-->prismapi
    prismapi--api-->RustSdk
    prismimpl--api-->RustSdk
...
This Android project mainly handle the application layer of the whole software. The communication with the PRISM server, as well as the local storage, the
...
[Renovate](https://github.com/apps/renovate) is set up on the project. This tool will automatically create Pull Request to upgrade our dependencies one by one. A [dependency dashboard issue](https://github.com/prism-hq/prism-x-android/issues/150) is maintained by the tool and allow to perform some actions.
...
- Combination of [Showkase](https://github.com/airbnb/Showkase) and [Paparazzi](https://github.com/cashapp/paparazzi), to test UI pixel perfect. To add test,
  ...
  file [TemplateView.kt](../features/template/src/main/kotlin/io/prism/android/features/template/TemplateView.kt). We create PreviewProvider to provide
  different states. See for instance the
  file [TemplateStateProvider.kt](../features/template/src/main/kotlin/io/prism/android/features/template/TemplateStateProvider.kt)
- Tests on presenter with [Molecule](https://github.com/cashapp/molecule) and [Turbine](https://github.com/cashapp/turbine). See in the template the class [TemplatePresenterTests](../features/template/src/test/kotlin/io/prism/android/features/template/TemplatePresenterTests.kt).
...
> actions. Use with caution and only if you are instructed by someone at PRISM or are
> already familiar.
...
The developer options include feature flags, notification/push history, PRISM call
