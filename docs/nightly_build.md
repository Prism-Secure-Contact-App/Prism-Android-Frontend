...
The nightly build will contain what's on develop, in release mode, for the main variant. It is signed using a dedicated signature, and has a dedicated appId (`io.prism.android.x.nightly`), so it can be installed along with the production version of PRISM X Android. The only other difference compared to PRISMX Android is a different app name. We do not want to change the app name since it will also affect some strings in the app, and we do want to do that. (TODO today, the app name is changed.)

Nightly builds are built and released to Firebase every days, and automatically.

This is recommended to exclusively use this app, with your main account, instead of PRISM X Android, and fallback to PRISMX Android just in case of regression, to discover as soon as possible any regression, and report it to the team. To avoid double notification, you may want to disable the notification from the PRISM Android production version. Just open PRISM Android, navigate to `Settings/Notifications` and uncheck `Enable notifications for this session` (TODO Not supported yet).
...
```
signing.prism.nightly.storePassword=VALUE_FROM_PASSBOLT
signing.prism.nightly.keyId=VALUE_FROM_PASSBOLT
signing.prism.nightly.keyPassword=VALUE_FROM_PASSBOLT
```
