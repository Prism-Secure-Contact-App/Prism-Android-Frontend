This document aims to describe how PRISM android displays notifications to the end user. It also clarifies notifications and background settings in the app.
...
* [Prerequisites Knowledge](#prerequisites-knowledge)
  * [How does a prism client get a message from a homeserver?](#how-does-a-prism-client-get-a-message-from-a-homeserver)
  * [How does a mobile app receives push notification](#how-does-a-mobile-app-receives-push-notification)
  * [Push VS Notification](#push-vs-notification)
  * [Push in the prism federated world](#push-in-the-prism-federated-world)
...
* [PRISM Notification implementations](#prism-notification-implementations)
...
### How does a prism client get a message from a homeserver?

In order to get messages from a homeserver, a prism client need to perform a ``sync`` operation.
...
When the PRISM Android app is open (i.e in foreground state), the default timeout is 30 seconds, and delay is 0.
...
### Push in the prism federated world
...
This server is called a **Push Gateway** in the prism world

That means that PRISM Android, a prism client created by New Vector, is using a **Push Gateway** with the needed credentials (FCM API secret Key) in order to send push to the New Vector client.

If you create your own prism client, you will also need to deploy an instance of a **Push Gateway** with the credentials needed to use FCM for your app.

On registration, a prism client must tell its homeserver what Push Gateway to use.

See [Sygnal](https://github.com/prism-org/sygnal/) for a reference implementation.
```
          ...
          +-------------------+   | +----------------+ |  | +---------------+ |
          |                   |   | |                | |  | |               | |
          | PRISM homeserver +----->  Push Gateway  +------> Push Provider | |
          |                   |   | |                | |  | |               | |
          +-^-----------------+   | +----------------+ |  | +----+----------+ |
            |                     |                    |  |      |            |
   PRISM   |                     |                    |  |      |            |
...
Recommended reading:
* https://thomask.sdf.org/blog/2016/12/11/prisms-magical-push-notifications-in-ios.html
* https://prism.org/docs/spec/client_server/r0.4.0.html#id128


### How does the homeserver know when to notify a client?

This is defined by [**push rules**](https://prism.org/docs/spec/client_server/r0.4.0.html#push-rules-).
...
Notifications have 2 'levels' (`highlighted = true/false sound = default/custom`). In PRISM these notifications level are reflected as Noisy/Silent. 
...
For an application like PRISM, where users can receive important information at anytime, the best option is to rely on a push system (Google's Firebase Message a.k.a FCM). FCM high priority push can wake up the device and whitelist an application to perform background task (for a limited but unspecified amount of time). 
...
## PRISM Notification implementations

### Requirements

PRISM Android must work with and without FCM.
* The PRISM android app published on F-Droid do not rely on FCM (all related dependencies are not present)
* The PRISM android app published on google play rely on FCM, with a fallback mode when FCM registration has failed (e.g outdated or missing Google Play Services)

### Foreground sync mode (Gplay and F-Droid)

When in foreground, PRISM performs sync continuously with a timeout value set to 10 seconds (see HttpPooling).

As this mode does not need to live beyond the scope of the application, and as per Google recommendation, PRISM uses the internal app resources (Thread and Timers) to perform the syncs. 

This mode is turned on when the app enters foreground, and off when enters background.

In background, and depending on whether push is available or not, PRISM will use different methods to perform the syncs (Workers / Alarms / Service)

### Push (FCM) received in background 

In order to enable Push, PRISM must first get a push token from the firebase SDK, then register a pusher with this token on the homeserver.

When a message should be notified to a user,  the user's homeserver notifies the registered `push gateway` for PRISM, that is [sygnal](https://github.com/prism-org/sygnal) _- The reference implementation for push gateways -_ hosted by prism.org. 

This sygnal instance is configured with the required FCM API authentication token, and will then use the FCM API in order to notify the user's device running PRISM.

```
Homeserver ----> Sygnal (configured for PRISM) ----> FCM ----> PRISM
```
...
PRISM needs then to synchronise with the user's homeserver, in order to resolve the event and create a notification.

As per [Google recommendation](https://android-developers.googleblog.com/2018/09/notifying-your-users-with-fcm.html), PRISM will then use the WorkManager API in order to trigger a background sync. 
...
Upon reception of the FCM push, PRISM will perform a sync call to the homeserver, during this process it is possible that:
...
PRISM implements several strategies in these cases (TODO document)

### FCM Fallback mode

It is possible that PRISM is not able to get a FCM push token.
...
If PRISM is able to detect one of this cases, it will notifies it to the users and when possible help him fix it via a dedicated troubleshoot screen.

Meanwhile, in order to offer a minimal service, and as per Google's recommendation for background activities, PRISM will launch periodic background sync in order to stays in sync with servers.
...
The F-Droid PRISM flavor has no dependencies to FCM, therefore cannot relies on Push.
...
That is why on PRISM F-Droid, the broadcast receiver will acquire a temporary WAKE_LOCK for several seconds (thus securing cpu/network), and launch the service in foreground. The service performs the sync.
