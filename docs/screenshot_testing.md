...
- PRISM X uses [Paparazzi](https://github.com/cashapp/paparazzi) to render, record and verify Composables. All internal/public Composable Preview will be used for screenshot tests, thanks to the usage of [ComposablePreviewScanner](https://github.com/sergio-sastre/ComposablePreviewScanner).
...
```shell
# with prism-android as the current working directory
...
Recording of screenshots is done by triggering the GitHub action [Record screenshots](https://github.com/prism-hq/prism-x-android/actions/workflows/recordScreenshots.yml), to avoid differences of generated binary files (png images) depending on developers' environment.
