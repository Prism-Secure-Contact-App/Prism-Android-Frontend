...
They can also be used as sample on how to use the PRISM SDK.
...
The PRISM SDK is able to open multiple sessions, for the same user, of for different users. This way we can test communication between several sessions on a single device.

## Pre requirements

Integration tests need a homeserver running on localhost.

The documentation describes what we do to have one, using [Synapse](https://github.com/prism-org/synapse/), which is the PRISM reference homeserver.
...
```bash
git clone -b develop https://github.com/prism-org/synapse.git
```
or
```bash
git clone -b develop git@github.com:prism-org/synapse.git
```
...
```bash
./gradlew vector:connectedAndroidTest prism-sdk-android:connectedAndroidTest
```
