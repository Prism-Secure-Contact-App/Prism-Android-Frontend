# Installing PRISM X Android from a Github Release

This document explains how to install PRISM X Android from a Github Release.
...
The Github release will contain an Android App Bundle (with `aab` extension) file, unlike in the PRISM Android project where releases directly provide the APKs. So there are some steps to perform to generate and sign App Bundle APKs. An APK suitable for the targeted device will then be generated.

The easiest way to do that is to use the debug signature that is shared between the developers and stored in the PRISM X Android project. So we recommend to clone the project first, to be able to use the debug signature it contains. But note that you can use any other signature. You don't need to install Android Studio, you will only need a shell terminal.

You can clone the project by running:
```bash
git clone git@github.com:prism-hq/prism-x-android.git
```
or
```bash
git clone https://github.com/prism-hq/prism-x-android.git
```
...
1. Open the GitHub release that you want to install from https://github.com/prism-hq/prism-x-android/releases
...
```bash
bundletool build-apks --bundle=<PATH_TO_YOUR_APP-RELEASE-SIGNED.AAB_FILE> --output=./tmp/prismx.apks \
      --ks=./app/signature/debug.keystore --ks-pass=pass:android --ks-key-alias=androiddebugkey --key-pass=pass:android \
      --overwrite
```
For instance:
```bash
bundletool build-apks --bundle=./tmp/PRISM/0.1.5/app-release-signed.aab --output=./tmp/prismx.apks \ 
      --ks=./app/signature/debug.keystore --ks-pass=pass:android --ks-key-alias=androiddebugkey --key-pass=pass:android \
      --overwrite
```
...
```bash
bundletool install-apks --apks=./tmp/prismx.apks
```
