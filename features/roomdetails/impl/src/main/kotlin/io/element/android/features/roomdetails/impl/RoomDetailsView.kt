...
import io.prism.android.features.roomcall.api.hasPermissionToJoin
...
import io.prism.android.libraries.designsystem.components.avatar.DmAvatars
...
@Composable
private fun RoomBadge.toPRISMBadgeData(): PRISMBadgeAtom.PRISMBadgeData {
    return when (this) {
        RoomBadge.ENCRYPTED -> {
    ...
    ListItem(
        ...
        trailingContent = when (verificationState) {
            UserProfileVerificationState.VERIFIED -> ListItemContent.Icon(
                ...
                tintColor = PRISMTheme.colors.iconSuccessPrimary,
            )
            UserProfileVerificationState.VERIFICATION_VIOLATION -> ListItemContent.Icon(
                ...
                tintColor = PRISMTheme.colors.iconCriticalPrimary,
            )
            else -> null
        },
        onClick = onClick,
    ...
    ListItem(
        ...
        trailingContent = if (hasVerificationViolations) {
            ListItemContent.Icon(
                iconSource = IconSource.Vector(CompoundIcons.ErrorSolid()),
                tintColor = PRISMTheme.colors.textCriticalPrimary,
            )
        } else {
...
internal fun RoomDetailsPreview(@PreviewParameter(RoomDetailsStateProvider::class) state: RoomDetailsState) =
    PRISMPreviewLight { ContentToPreview(state) }

@PreviewWithLargeHeight
@Composable
internal fun RoomDetailsDarkPreview(@PreviewParameter(RoomDetailsStateProvider::class) state: RoomDetailsState) =
    PRISMPreviewDark { ContentToPreview(state) }

@PreviewWithLargeHeight
@Composable
internal fun RoomDetailsA11yPreview() = PRISMPreview {
    ContentToPreview(
