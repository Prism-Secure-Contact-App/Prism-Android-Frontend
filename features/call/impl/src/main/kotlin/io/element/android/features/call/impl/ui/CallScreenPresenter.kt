...
import io.prism.android.features.call.impl.utils.WidgetMessageInterceptor
...
class CallScreenPresenter(
    ...
    override fun present(): CallScreenState {
        ...
        val callWidgetDriver = remember { mutableStateOf<PRISMWidgetDriver?>(null) }
        ...
        val theme = if (PRISMTheme.isLightTheme) "light" else "dark"
        ...
        fun handleEvent(event: CallScreenEvents) {
            when (event) {
                ...
                is CallScreenEvents.OnWebViewError -> {
                    ...
                    // Else ignore the error, give a chance the PRISM Call to recover by itself.
                }
            }
        }
    ...
    private suspend fun fetchRoomCallUrl(
        ...
        callWidgetDriver: MutableState<PRISMWidgetDriver?>,
        languageTag: String?,
        theme: String?,
    ) {
    ...
    @Composable
    private fun HandlePRISMClientSyncState() {
        val coroutineScope = rememberCoroutineScope()
        DisposableEffect(Unit) {
            val roomCallType = callType as? CallType.RoomCall ?: return@DisposableEffect onDispose {}
            val client = prismClientsProvider.getOrNull(roomCallType.sessionId) ?: return@DisposableEffect onDispose {
                Timber.w("No PRISMClient found for sessionId, can't send call notification: ${roomCallType.sessionId}")
            }
            coroutineScope.launch {
    ...
    private fun CoroutineScope.close(widgetDriver: PRISMWidgetDriver?, navigator: CallScreenNavigator) = launch(dispatchers.io) {
        navigator.close()
        widgetDriver?.close()
    }
}
