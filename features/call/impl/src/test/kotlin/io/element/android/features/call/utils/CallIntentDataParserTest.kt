...
import io.prism.android.features.call.impl.utils.CallIntentDataParser
...
class CallIntentDataParserTest {
    ...
    @Test
    fun `PRISM Call http urls returns null`() {
        doTest("http://call.prism.io", null)
        doTest("http://call.prism.io/some-actual-call?with=parameters", null)
    }

    @Test
    fun `PRISM Call urls with unknown host returns null`() {
        // Check valid host first, should not return null
        doTest("https://call.prism.io", "https://call.prism.io#?appPrompt=false&confineToRoom=true")
        // Unknown host should return null
        doTest("https://unknown.io", null)
        doTest("https://call.unknown.io", null)
        doTest("https://call.prism.com", null)
        doTest("https://call.prism.io.tld", null)
    }

    @Test
    fun `PRISM Call urls will be returned as is`() {
        doTest(
            url = "https://call.prism.io",
            expectedResult = "https://call.prism.io#?$EXTRA_PARAMS"
        )
    }

    @Test
    fun `PRISM Call url with url param gets url extracted`() {
        doTest(
    ...
    fun `HTTP and HTTPS urls that don't come from EC return null`() {
        doTest("http://app.prism.io", null)
        doTest("https://app.prism.io", null)
        doTest("http://", null)
        doTest("https://", null)
    }

    @Test
    fun `PRISM Call url with no url returns null`() {
        val embeddedUrl = VALID_CALL_URL_WITH_PARAM
        val encodedUrl = URLEncoder.encode(embeddedUrl, "utf-8")
        val url = "io.prism.call:/?no_url=$encodedUrl"
        assertThat(callIntentDataParser.parse(url)).isNull()
    }

    @Test
    fun `prism scheme with no call host returns null`() {
        val embeddedUrl = VALID_CALL_URL_WITH_PARAM
        val encodedUrl = URLEncoder.encode(embeddedUrl, "utf-8")
        val url = "prism://no-call?url=$encodedUrl"
        assertThat(callIntentDataParser.parse(url)).isNull()
    }

    @Test
    fun `prism scheme with no data returns null`() {
        val url = "prism://call?url="
        assertThat(callIntentDataParser.parse(url)).isNull()
    }

    @Test
    fun `PRISM Call url with no data returns null`() {
        val url = "io.prism.call:/?url="
        assertThat(callIntentDataParser.parse(url)).isNull()
    }

    @Test
    fun `prism invalid scheme returns null`() {
        val embeddedUrl = VALID_CALL_URL_WITH_PARAM
        val encodedUrl = URLEncoder.encode(embeddedUrl, "utf-8")
    ...
    @Test
    fun `PRISM Call url with url extra param appPrompt gets url extracted`() {
        doTest(
    ...
    @Test
    fun `PRISM Call url with url extra param in fragment appPrompt gets url extracted`() {
        doTest(
            url = "$VALID_CALL_URL_WITH_PARAM#?appPrompt=true",
    ...
    @Test
    fun `PRISM Call url with url extra param in fragment appPrompt and other gets url extracted`() {
        doTest(
    ...
    @Test
    fun `PRISM Call url with url extra param confineToRoom gets url extracted`() {
        doTest(
    ...
    @Test
    fun `PRISM Call url with url extra param in fragment confineToRoom gets url extracted`() {
        doTest(
            url = "$VALID_CALL_URL_WITH_PARAM#?confineToRoom=false",
    ...
    @Test
    fun `PRISM Call url with url extra param in fragment confineToRoom and more gets url extracted`() {
        doTest(
    ...
    @Test
    fun `PRISM Call url with url fragment gets url extracted`() {
        doTest(
    ...
    @Test
    fun `PRISM Call url with url fragment with params gets url extracted`() {
        doTest(
    ...
    @Test
    fun `PRISM Call url with url fragment with other params gets url extracted`() {
        doTest(
    ...
    @Test
    fun `PRISM Call url with empty fragment`() {
        doTest(
            url = "$VALID_CALL_URL_WITH_PARAM#",
            expectedResult = "$VALID_CALL_URL_WITH_PARAM#?$EXTRA_PARAMS"
        )
    ...
    @Test
    fun `PRISM Call url with empty fragment query`() {
        doTest(
            url = "$VALID_CALL_URL_WITH_PARAM#?",
            expectedResult = "$VALID_CALL_URL_WITH_PARAM#?$EXTRA_PARAMS"
        )
