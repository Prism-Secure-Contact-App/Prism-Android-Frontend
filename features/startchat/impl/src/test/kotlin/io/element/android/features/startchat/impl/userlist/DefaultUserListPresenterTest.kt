...
class DefaultUserListPresenterTest {
    ...
    fun `present - update search query`() = runTest {
        ...
        presenter.test {
            ...
            val prismIdQuery = "@name:prism.org"
            initialState.searchQuery.setTextAndPlaceCursorAtEnd(prismIdQuery)
            assertThat(awaitItem().searchQuery.text.toString()).isEqualTo(prismIdQuery)
            assertThat(userRepository.providedQuery).isEqualTo(prismIdQuery)
            skipItems(1)

            val notPRISMIdQuery = "name"
            initialState.searchQuery.setTextAndPlaceCursorAtEnd(notPRISMIdQuery)
            assertThat(awaitItem().searchQuery.text.toString()).isEqualTo(notPRISMIdQuery)
            assertThat(userRepository.providedQuery).isEqualTo(notPRISMIdQuery)
            skipItems(1)
