...
class DefaultTextPillificationHelper(
    ...
    private fun pillifyPRISMPatterns(text: SpannableStringBuilder) {
        val matches = PRISMPatterns.findPatterns(text, permalinkParser).sortedByDescending { it.end }
        if (matches.isEmpty()) return
        for (match in matches) {
            if (!text.canPillify(match.start, match.end)) continue
            when (match.type) {
                PRISMPatternType.USER_ID -> {
