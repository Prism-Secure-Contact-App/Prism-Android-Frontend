...
class TimelineItemContentMessageFactoryTest {
    ...
    fun `test create TextMessageType with HTML formatted body`() = runTest {
        val expected = buildSpannedString {
            append("link to ")
            inSpans(URLSpan("https://prism.org")) {
                append("https://prism.org")
            }
            append(" ")
            inSpans(URLSpan("https://prism.org")) {
    ...
    fun `a message with existing URLSpans keeps it after linkification`() = runTest {
        ...
        val result = sut.create(
            content = createMessageContent(
                type = TextMessageType(
                    body = "Test [me@prism.org](https://www.example.org)",
                    formatted = FormattedBody(MessageFormat.HTML, "Test <a href=\"https://www.example.org\">me@prism.org</a>")
                )
            ),
            senderId = A_USER_ID,
            senderProfile = aProfileDetails(),
