...
import io.prism.android.features.messages.impl.FakeMessagesNavigator
import io.prism.android.features.messages.impl.crypto.sendfailure.resolve.aResolveVerifiedUserSendFailureState
import io.prism.android.features.messages.impl.fixtures.aMessageEvent
...
import io.prism.android.features.messages.impl.typing.aTypingNotificationState
import io.prism.android.features.messages.impl.voicemessages.timeline.FakeRedactedVoiceMessageManager
import io.prism.android.features.messages.impl.voicemessages.timeline.RedactedVoiceMessageManager
import io.prism.android.features.messages.impl.voicemessages.timeline.aRedactedPRISMTimeline
...
import io.prism.android.features.poll.test.actions.FakeEndPollAction
...
class TimelinePresenterTest {
    ...
    private fun `present - on scroll finished mark a room as read if the first visible index is 0`(
        ...
        val timeline = FakeTimeline(
            timelineItems = flowOf(
                listOf(
                    PRISMTimelineItem.Event(A_UNIQUE_ID, anEventTimelineItem())
                )
            ),
    ...
    fun `present - on scroll finished will not send read receipt only virtual events exist before the index`() = runTest {
        ...
        val timeline = FakeTimeline(
            timelineItems = flowOf(
                listOf(
                    PRISMTimelineItem.Virtual(A_UNIQUE_ID, VirtualTimelineItem.ReadMarker),
                    PRISMTimelineItem.Virtual(A_UNIQUE_ID, VirtualTimelineItem.ReadMarker)
                )
            )
        ).apply {
    ...
    fun `present - covers newEventState scenarios`() = runTest {
        val timelineItems = MutableStateFlow(emptyList<PRISMTimelineItem>())
        val timeline = FakeTimeline(
        ...
        presenter.test {
            ...
            timelineItems.emit(
                listOf(PRISMTimelineItem.Event(UniqueId("0"), anEventTimelineItem(content = aMessageContent())))
            )
            ...
            timelineItems.getAndUpdate { items ->
                ...
                items + listOf(PRISMTimelineItem.Event(UniqueId("1"), event))
            }
            ...
            timelineItems.getAndUpdate { items ->
                ...
                items + listOf(PRISMTimelineItem.Event(UniqueId("2"), event))
            }
            ...
            timelineItems.getAndUpdate { items ->
                val event = anEventTimelineItem(content = aMessageContent())
                items + listOf(PRISMTimelineItem.Event(UniqueId("3"), event))
            }
    ...
    fun `present - reaction ordering`() = runTest {
        ...
        presenter.test {
            ...
            timelineItems.emit(
                listOf(PRISMTimelineItem.Event(A_UNIQUE_ID, anEventTimelineItem(reactions = oneReaction)))
            )
    ...
    fun `present - side effect on redacted items is invoked`() = runTest {
        ...
        val presenter = createTimelinePresenter(
            timeline = FakeTimeline(
                timelineItems = flowOf(
                    aRedactedPRISMTimeline(AN_EVENT_ID),
                )
            ),
