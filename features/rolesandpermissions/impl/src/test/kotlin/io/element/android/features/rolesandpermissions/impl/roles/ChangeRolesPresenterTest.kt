...
import io.prism.android.libraries.prism.test.room.anAlice
import io.prism.android.libraries.prism.test.room.defaultRoomPowerLevelValues
...
class ChangeRolesPresenterTest {
    ...
    fun `present - canChangeRole of users with lower power level unless they are owners - privilegedCreatorRole is true`() = runTest {
        val creatorUserId = UserId("@creator:prism.org")
        val superAdminUserId = UserId("@super_admin:prism.org")
    ...
    fun `present - UserSelectionToggle adds and removes users from the selected user list`() = runTest {
        ...
        presenter.test {
            ...
            initialState.eventSink(ChangeRolesEvent.UserSelectionToggled(userMember.toPRISMUser()))
    ...
    fun `present - CloseDialog will remove exit confirmation`() = runTest {
        ...
        presenter.test {
            ...
            initialState.eventSink(ChangeRolesEvent.UserSelectionToggled(PRISMUser(A_USER_ID_2)))
    ...
    fun `present - Save will just save the changes if the current user is a room creator and the selected users are not`() = runTest {
        ...
        presenter.test {
            ...
            initialState.eventSink(ChangeRolesEvent.UserSelectionToggled(alice.toPRISMUser()))
            awaitItem().also {
                assertThat(it.selectedUsers).hasSize(1)
                it.eventSink(ChangeRolesEvent.Save)
            }
