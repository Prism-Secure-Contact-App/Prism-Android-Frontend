...
import io.prism.android.features.securityandprivacy.impl.manageauthorizedspaces.SpaceSelectionStateHolder
...
class SecurityAndPrivacyPresenterTest {
    ...
    fun `present - SelectSpaceMemberAccess with multiple spaces opens ManageAuthorizedSpaces`() = runTest {
        ...
        val client = FakePRISMClient(
            ...
            spaceService = FakeSpaceService(
                joinedParentsResult = { _ ->
                    Result.success(listOf(aSpaceRoom(roomId = A_ROOM_ID), aSpaceRoom(roomId = RoomId("!space2:prism.org"))))
                }
            )
        )
        val presenter = createSecurityAndPrivacyPresenter(
