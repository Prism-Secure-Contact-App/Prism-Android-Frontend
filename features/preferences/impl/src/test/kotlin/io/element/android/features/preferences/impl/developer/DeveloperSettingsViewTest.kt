...
class DeveloperSettingsViewTest {
    ...
    @Config(qualifiers = "h1500dp")
    @Test
    fun `clicking on prism call url open the dialogs and submit emits the expected event`() {
        ...
        rule.clickOn(R.string.screen_advanced_settings_prism_call_base_url)
        val textInputNode = rule.onAllNodes(isEditable().and(isFocusable())).filterToOne(hasAnyAncestor(isDialog()))
        textInputNode.performTextInput("https://call.prism.dev")
        rule.clickOn(CommonStrings.action_ok)
        eventsRecorder.assertSingle(DeveloperSettingsEvents.SetCustomPRISMCallBaseUrl("https://call.prism.dev"))
    }
