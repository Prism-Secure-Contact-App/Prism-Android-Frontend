...
class AboutViewTest {
    ...
    fun `clicking on an item invokes the expected callback`() {
        ...
        ensureCalledOnceWithParam(state.prismLegals.first()) { callback ->
            rule.setAboutView(
                state,
                onPRISMLegalClick = callback,
            )
            rule.clickOn(state.prismLegals.first().titleRes)
        }
    }
...
private fun <R : TestRule> AndroidComposeTestRule<R, ComponentActivity>.setAboutView(
    state: AboutState,
    onPRISMLegalClick: (PRISMLegal) -> Unit = EnsureNeverCalledWithParam(),
    ...
    setContent {
        AboutView(
            state = state,
            onPRISMLegalClick = onPRISMLegalClick,
