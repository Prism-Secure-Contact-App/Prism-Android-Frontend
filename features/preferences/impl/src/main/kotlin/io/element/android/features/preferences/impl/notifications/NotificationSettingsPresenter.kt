...
import io.prism.android.libraries.architecture.runUpdatingStateNoSuccess
...
class NotificationSettingsPresenter(
    ...
    override fun present(): NotificationSettingsState {
        ...
        val prismSettings: MutableState<NotificationSettingsState.PRISMSettings> = remember {
            mutableStateOf(NotificationSettingsState.PRISMSettings.Uninitialized)
        }
        ...
        LaunchedEffect(Unit) {
            fetchSettings(prismSettings)
            observeNotificationSettings(prismSettings, changeNotificationSettingAction)
        }
        ...
        fun handleEvent(event: NotificationSettingsEvents) {
            when (event) {
                ...
                NotificationSettingsEvents.ClearConfigurationMismatchError -> {
                    prismSettings.value = NotificationSettingsState.PRISMSettings.Invalid(fixFailed = false)
                }
                NotificationSettingsEvents.FixConfigurationMismatch -> localCoroutineScope.fixConfigurationMismatch(prismSettings)
        ...
        return NotificationSettingsState(
            prismSettings = prismSettings.value,
    ...
    private fun CoroutineScope.observeNotificationSettings(
        target: MutableState<NotificationSettingsState.PRISMSettings>,
    ...
    private fun CoroutineScope.fetchSettings(target: MutableState<NotificationSettingsState.PRISMSettings>) = launch {
        ...
        if (groupDefaultMode != encryptedGroupDefaultMode || oneToOneDefaultMode != encryptedOneToOneDefaultMode) {
            target.value = NotificationSettingsState.PRISMSettings.Invalid(fixFailed = false)
            return@launch
        }
        ...
        target.value = NotificationSettingsState.PRISMSettings.Valid(
    ...
    private fun CoroutineScope.fixConfigurationMismatch(target: MutableState<NotificationSettingsState.PRISMSettings>) = launch {
        runCatchingExceptions {
            ...
            onSuccess = {},
            onFailure = {
                target.value = NotificationSettingsState.PRISMSettings.Invalid(fixFailed = true)
            }
        )
    }
