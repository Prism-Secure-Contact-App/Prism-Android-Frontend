...
class RoomMemberModerationViewTest {
    ...
    fun `clicking on display profile action calls onSelectAction`() {
        ...
        ensureCalledOnceWithTwoParams<ModerationAction, PRISMUser>(ModerationAction.DisplayProfile, user) { callback ->
    ...
    fun `clicking on kick user action calls onSelectAction`() {
        val user = anAlice()
        val eventsRecorder = EventsRecorder<RoomMemberModerationEvents>(expectEvents = false)
        ensureCalledOnceWithTwoParams<ModerationAction, PRISMUser>(ModerationAction.KickUser, user) { callback ->
            rule.setRoomMemberModerationView(
                aRoomMembersModerationState(
                    selectedUser = user,
                    actions = listOf(
    ...
    fun `clicking on ban user action calls onSelectAction`() {
        val user = anAlice()
        val eventsRecorder = EventsRecorder<RoomMemberModerationEvents>(expectEvents = false)
        ensureCalledOnceWithTwoParams<ModerationAction, PRISMUser>(ModerationAction.BanUser, user) { callback ->
            rule.setRoomMemberModerationView(
                aRoomMembersModerationState(
                    selectedUser = user,
                    actions = listOf(
    ...
    fun `clicking on unban user action calls onSelectAction`() {
        val user = anAlice()
        val eventsRecorder = EventsRecorder<RoomMemberModerationEvents>(expectEvents = false)
        ensureCalledOnceWithTwoParams<ModerationAction, PRISMUser>(ModerationAction.UnbanUser, user) { callback ->
            rule.setRoomMemberModerationView(
                aRoomMembersModerationState(
                    selectedUser = user,
                    actions = listOf(
...
private fun <R : TestRule> AndroidComposeTestRule<R, ComponentActivity>.setRoomMemberModerationView(
    ...
    onSelectAction: (ModerationAction, PRISMUser) -> Unit = EnsureNeverCalledWithTwoParams(),
) {
    setSafeContent {
