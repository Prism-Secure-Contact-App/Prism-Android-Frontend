 ...
 * Ex for logcat:
 * `01-23 13:14:50.740 25818 25818 D org.prism.rust.sdk: prismx: SyncIndicator = Hide | RustRoomListService.kt:81`
 *                                 ^ use this char to determine the color
 * Ex for Rust logs:
 * `2024-01-26T10:22:26.947416Z  WARN prismx: Restore with non-empty map | PRISMClientsHolder.kt:68`
...
private fun String.toColor(colorationMode: ColorationMode): Color {
    return when (colorationMode) {
        ColorationMode.Logcat -> when (getOrNull(31)) {
            ...
            'E' -> colorError
            'A' -> colorError
            else -> PRISMTheme.colors.textPrimary
        }
        ColorationMode.RustLogs -> when (getOrNull(32)) {
            'E' -> PRISMTheme.colors.textPrimary
            'G' -> colorDebug
            'O' -> colorInfo
            'N' -> colorWarning
            'R' -> colorError
        ...
        ColorationMode.None -> PRISMTheme.colors.textPrimary
    }
}
