...
import io.prism.android.features.enterprise.test.FakeSessionEnterpriseService
...
class UserProfilePresenterTest {
    ...
    fun `present - canCall is false when call is not available`() {
        testCanCall(
            isPRISMCallAvailable = false,
            expectedResult = false,
        )
    }

    private fun testCanCall(
        isPRISMCallAvailable: Boolean = true,
        ...
        val presenter = createUserProfilePresenter(
            userId = A_USER_ID_2,
            client = client,
            isPRISMCallAvailable = isPRISMCallAvailable,
        )
        presenter.test {
    ...
    fun `present - BlockUser and UnblockUser without confirmation change the 'blocked' state`() = runTest {
        ...
        val client = createFakePRISMClient(ignoredUsersFlow = ignoredUsersFlow)
        val presenter = createUserProfilePresenter(
            client = client,
            userId = A_USER_ID
        )
    ...
    private fun createFakePRISMClient(
    ...
    private fun createUserProfilePresenter(
        client: PRISMClient = createFakePRISMClient(),
        ...
        return UserProfilePresenter(
            ...
            sessionEnterpriseService = FakeSessionEnterpriseService(
                isPRISMCallAvailableResult = { isPRISMCallAvailable },
            ),
        )
    }
}
