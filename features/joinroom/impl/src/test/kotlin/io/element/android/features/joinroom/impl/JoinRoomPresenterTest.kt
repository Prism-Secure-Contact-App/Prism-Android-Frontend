...
import io.prism.android.features.invite.api.toInviteData
...
class JoinRoomPresenterTest {
            ...
            getNotJoinedRoomResult = { _, _ ->
                Result.success(
                    aRoomPreview(
                        info = aRoomPreviewInfo(
                            roomId = A_ROOM_ID,
                            canonicalAlias = RoomAlias("#alias:prism.org"),
                            name = "Room name",
                            topic = "Room topic",
                            avatarUrl = "avatarUrl",
            ...
            awaitItem().also { state ->
                assertThat(state.contentState).isEqualTo(
                    ContentState.Loaded(
                        ...
                        name = "Room name",
                        topic = "Room topic",
                        alias = RoomAlias("#alias:prism.org"),
                        numberOfMembers = 2,
    ...
    fun `present - when room is not known RoomPreview is loaded with error Forbidden`() = runTest {
        val client = FakePRISMClient(
            getNotJoinedRoomResult = { _, _ ->
                Result.failure(ClientException.PRISMApi(ErrorKind.Forbidden, "403", "Forbidden", null))
            },
            spaceService = FakeSpaceService(
                spaceRoomListResult = { FakeSpaceRoomList() },
            ),
    ...
    private fun aRoomDescription(
        ...
        alias: RoomAlias? = RoomAlias("#alias:prism.org"),
