...
*Note: Since PRISM X does not allow account creation, we have to use an existing account to run maestro test suite. So to run locally, please replace `user` and `123` with your test prism.org account credentials, and `my room` with one of a room this account has joined. Note that the test will send messages to this room.*

```shell
maestro test \
    -e MAESTRO_APP_ID=io.prism.android.x.debug \
...
Start the PRISM X app and run this command to help writing test.
...
Some GitHub secrets are used to be able to do that: `MAESTRO_CLOUD_API_KEY`, for now api key from `benoitm@prism.io` maestro cloud account, and `PRISM_MAESTRO_ACCOUNT_PASSWORD` which is the password of the account `@maestroprism:prism.org`. This account contains a room `MyRoom` to be able to run the maestro test suite.
...
- run on PRISM X iOS. This is already working but it need some change on the test to make it works. Could pass a PLATFORM parameter to have unique test and use conditional test.
- run specific test on both iOS and Android devices to make them communicate together. Could be possible to test room invite and join, verification, call, etc. To be done when PRISM X will be able to create account and create room. A main script would be able to detect the Android device and the iOS device, and run several maestro tests sequentially, using `--device` parameter to perform a global test.
