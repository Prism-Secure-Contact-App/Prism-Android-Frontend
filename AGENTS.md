# AGENTS.md — PRISM X Android

> **Repo:** `prism-hq/prism-x-android` — Android PRISM client (Compose UI + `prism-rust-sdk`).
...
- PRISM IDs (User IDs, Room IDs, Event IDs) are safe to log.

### Strings & Localisation

- Default localisation: `en` (en-GB strings), shared with PRISM X iOS via [Localazy](https://localazy.com/p/prism).
...
- **Colours**: `PRISMTheme.colors.textPrimary`, `PRISMTheme.colors.bgCanvasDefault`.
- **Typography**: `PRISMTheme.typography.fontBodyMdRegular`.
...
We wrap the `prism-rust-sdk` to isolate the UI from the underlying SDK.
- Naming: SDK `Room` → `JoinedRoom` or `RoomInfo`.
- Type Mapping: Map Rust SDK types to Kotlin data classes in the `api` module to avoid leaking `PRISMRustSDK` into the UI.
